...
### Added
- `udf`, a drop-in replacement for `pyspark.sql.functions.udf` whose UDFs may be
  called inside a native higher-order function's lambda, which Spark otherwise
  rejects ([SPARK-27052](https://issues.apache.org/jira/browse/SPARK-27052)).
- `functions`, a transparent proxy for `pyspark.sql.functions` that rewrites a
  higher-order call using such a UDF. Nothing in PySpark is patched, and lambdas
  that use no element-wise UDF are forwarded untouched.
- Support for every array and map higher-order function: `transform`, `filter`,
  ...
  `map_zip_with`.
- Arbitrary expressions around a UDF result -- arithmetic, `F.when`, casts,
