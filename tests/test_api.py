...
def test_bare_decorator_defaults_to_string(spark):
    ...
    assert [r[0] for r in df.select(esf.transform("v", lambda x: stringify(x))).collect()] == [
        ["1", "2"]
    ]
...
def test_called_directly_with_function_and_type(spark):
    ...
    df = spark.createDataFrame([([1, 2],)], "v array<int>")
    assert [r[0] for r in df.select(esf.transform("v", lambda x: plus_one(x))).collect()] == [
        [2, 3]
    ]


def test_use_arrow(spark):
    plus_one = esf.udf(lambda x: x + 1, "long", useArrow=True)
    df = spark.createDataFrame([([1, 2],)], "v array<int>")
    assert [r[0] for r in df.select(esf.transform("v", lambda x: plus_one(x))).collect()] == [
        [2, 3]
    ]
...
def test_as_nondeterministic(spark):
    plus_one = esf.udf(lambda x: x + 1, "long").asNondeterministic()
...
def test_ddl_and_datatype_return_types_agree(spark):
    ...
    df = spark.createDataFrame([([1, 2],)], "v array<int>")
    ddl = df.select(esf.transform("v", lambda x: from_ddl(x))).collect()
    typed = df.select(esf.transform("v", lambda x: from_datatype(x))).collect()
    assert ddl == typed
...
def test_struct_return_type(spark):
    @esf.udf("struct<doubled:long,text:string>")
    def to_struct(x):
        return (x * 2, str(x))

    df = spark.createDataFrame([([1, 2],)], "v array<int>")
    got = df.select(esf.transform("v", lambda x: to_struct(x)["doubled"])).collect()
...
def test_nested_array_elements(spark):
    ...
    assert [r[0] for r in df.select(esf.transform("v", lambda x: total(x))).collect()] == [[3, 3]]
