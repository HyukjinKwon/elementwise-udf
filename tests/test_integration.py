...
def regular_double(column):
    ...
    """
    return sf.udf(lambda x: x * 2, "long")(column)
...
def test_regular_udf_on_a_scalar_column_beside_a_higher_order_call(spark):
    ...
    out = df.select(
        ...
        esf.transform("v", lambda x: plus_one(x)).alias("mapped"),
    )
...
def test_regular_udf_applied_to_the_rewritten_array(spark):
    ...
    out = df.select(regular_double(sf.element_at(mapped, 1)).alias("first_doubled"))
    assert rows(out) == [(4,)]
...
def test_three_python_udfs_in_one_projection(spark):
    ...
    out = df.select(
        ...
        esf.transform("v", lambda x: plus_one(x)).alias("b"),
        esf.transform("v", lambda x: label(x)).alias("c"),
    )
...
def test_rewrite_survives_an_inner_join(spark):
    ...
    out = (
        left.join(right, "id")
        .select("id", "tag", esf.transform("v", lambda x: plus_one(x)).alias("mapped"))
        .orderBy("id")
    )
...
def test_rewrite_on_both_sides_of_a_join(spark):
    ...
    out = left.join(right, "id").select(
        esf.transform("v", lambda x: plus_one(x)).alias("a"),
        esf.transform("w", lambda x: plus_one(x)).alias("b"),
    )
...
def test_rewrite_in_a_join_condition(spark):
    ...
    mapped = esf.transform("v", lambda x: plus_one(x))
    out = left.join(right, sf.element_at(mapped, 1) == right["k"]).select("id", "k")
    assert rows(out) == [(1, 2)]
...
def test_left_outer_join_keeps_null_arrays_null(spark):
    ...
    out = (
        ...
        .select("id", esf.transform("v", lambda x: plus_one(x)).alias("mapped"))
        .orderBy("id")
    )
...
def test_rewrite_inside_a_group_by_aggregation(spark):
    ...
    mapped = esf.transform("v", lambda x: plus_one(x))
    out = df.groupBy("k").agg(sf.sum(sf.element_at(mapped, 1)).alias("total")).orderBy("k")
...
def test_rewrite_with_a_window_function(spark):
    ...
    out = df.select("k", sf.row_number().over(window).alias("rn"), mapped.alias("m")).orderBy(
        "k", "rn"
    )
...
def test_rewrite_in_order_by(spark):
    ...
    mapped = esf.transform("v", lambda x: plus_one(x))
    out = df.orderBy(sf.element_at(mapped, 1)).select(mapped.alias("m"))
...
def test_rewrite_in_a_filter_predicate(spark):
    ...
    mapped = esf.transform("v", lambda x: plus_one(x))
    out = df.where(sf.element_at(mapped, 1) > 3).select(mapped.alias("m"))
    assert rows(out) == [([6],)]
...
def test_union_of_two_rewrites(spark):
    ...
    out = left.select(esf.transform("v", lambda x: plus_one(x)).alias("m")).union(
        right.select(esf.transform("v", lambda x: plus_one(x)).alias("m"))
    )
...
def test_distinct_over_a_rewrite(spark):
    df = spark.createDataFrame([([1],), ([1],), ([2],)], "v array<int>")
    out = df.select(esf.transform("v", lambda x: plus_one(x)).alias("m")).distinct()
...
def test_chained_withcolumn_calls(spark):
    df = spark.createDataFrame([([1, 2],)], "v array<int>")
    out = (
        df.withColumn("a", esf.transform("v", lambda x: plus_one(x)))
        .withColumn("b", esf.transform("a", lambda x: plus_one(x)))
        .select("a", "b")
    )
...
def test_cached_dataframe(spark):
    df = spark.createDataFrame([([1, 2],)], "v array<int>")
    out = df.select(esf.transform("v", lambda x: plus_one(x)).alias("m")).cache()
    try:
        assert rows(out) == [([2, 3],)]
...
def test_written_and_read_back(spark, tmp_path):
    ...
    df.select(esf.transform("v", lambda x: plus_one(x)).alias("m")).write.parquet(target)
    assert rows(spark.read.parquet(target)) == [([2, 3],)]


def test_temp_view_and_repeated_collect(spark):
    df = spark.createDataFrame([([1, 2],)], "v array<int>")
    df.select(esf.transform("v", lambda x: plus_one(x)).alias("m")).createOrReplaceTempView(
        "elementwise_view"
    )
    try:
...
def test_explain_does_not_raise(spark):
    df = spark.createDataFrame([([1, 2],)], "v array<int>")
    df.select(esf.transform("v", lambda x: plus_one(x))).explain(extended=False)


def test_schema_is_array_of_the_declared_return_type(spark):
    df = spark.createDataFrame([([1, 2],)], "v array<int>")
    out = df.select(esf.transform("v", lambda x: plus_one(x)).alias("m"))
