...
def test_transform_keeps_alias(values):
    out = values.select(F.transform("v", lambda x: plus_one(x)).alias("result"))
    assert out.columns == ["result"]


def test_plain_udf_call_still_works(spark):
    assert col(spark.range(1), plus_one(F.lit(1))) == [2]


def test_bare_udf_instead_of_lambda(values):
    assert col(values, F.transform("v", plus_one)) == [[2, 3, 4], [], None]


def test_column_argument(values):
    assert col(values, F.transform(F.col("v"), lambda x: plus_one(x))) == [[2, 3, 4], [], None]
...
def test_arithmetic_on_udf_result(values):
    assert col(values, F.transform("v", lambda x: plus_one(x) * 2)) == [[4, 6, 8], [], None]


def test_udf_result_mixed_with_element_and_index(values):
    got = col(values, F.transform("v", lambda x, i: plus_one(x) * 100 + x + i))
    assert got == [[201, 303, 405], [], None]


def test_when_on_udf_result(values):
    got = col(values, F.transform("v", lambda x: F.when(plus_one(x) > 2, 1).otherwise(0)))
    assert got == [[0, 1, 1], [], None]


def test_cast_on_udf_result(values):
    got = col(values, F.transform("v", lambda x: plus_one(x).cast("string")))
    assert got == [["2", "3", "4"], [], None]


def test_two_udfs_in_one_lambda(values):
    got = col(values, F.transform("v", lambda x: plus_one(x) + times_ten(x)))
    assert got == [[12, 23, 34], [], None]


def test_udf_argument_is_an_expression(values):
    assert col(values, F.transform("v", lambda x: plus_one(x * 10))) == [[11, 21, 31], [], None]


def test_nested_udfs(values):
    assert col(values, F.transform("v", lambda x: plus_one(times_ten(x)))) == [
        [11, 21, 31],
        [],
        None,
    ]
...
def test_udf_called_with_keyword_argument(values):
    assert col(values, F.transform("v", lambda x: plus_one(x=x))) == [[2, 3, 4], [], None]


def test_captured_column_argument(spark):
    df = spark.createDataFrame([([1, 2], 10), ([3], 20)], "v array<int>, k int")
    got = col(df, F.transform("v", lambda x: add(x, F.col("k"))))
    assert got == [[11, 12], [23]]
...
def test_many_higher_order_calls_with_plain_columns(spark):
    ...
    out = df.select(
        "id",
        "tag",
        F.transform("v", lambda x: plus_one(x) + 1).alias("a"),
        F.transform("v", lambda x: plus_one(x) + 2).alias("b"),
        F.filter("v", lambda x: times_ten(x) > 15).alias("c"),
        F.exists("v", lambda x: plus_one(x) > 4).alias("d"),
    )
...
def test_nested_higher_order_calls(values):
    got = col(values, F.transform(F.filter("v", lambda x: is_odd(x)), lambda y: times_ten(y)))
...
def test_filter(values):
    assert col(values, F.filter("v", lambda x: is_odd(x))) == [[1, 3], [], None]


def test_filter_on_an_expression(values):
    assert col(values, F.filter("v", lambda x: plus_one(x) > 2)) == [[2, 3], [], None]


def test_exists(values):
    assert col(values, F.exists("v", lambda x: plus_one(x) > 3)) == [True, False, None]


def test_forall(values):
    assert col(values, F.forall("v", lambda x: plus_one(x) > 1)) == [True, True, None]
...
def test_zip_with(spark):
    ...
    assert col(df, F.zip_with("l", "r", lambda a, b: add(a, b))) == [[11, 22, 33], [10]]


def test_zip_with_expression(spark):
    df = spark.createDataFrame([([1, 2], [10, 20])], "l array<int>, r array<int>")
    assert col(df, F.zip_with("l", "r", lambda a, b: add(a, b) * 2)) == [[22, 44]]


def test_aggregate_on_element(values):
    got = col(values, F.aggregate("v", F.lit(0).cast("long"), lambda acc, x: acc + plus_one(x)))
    assert got == [9, 0, None]


def test_reduce_on_element(values):
    got = col(values, F.reduce("v", F.lit(0).cast("long"), lambda acc, x: acc + plus_one(x)))
    assert got == [9, 0, None]
...
def test_aggregate_with_finish_lambda(values):
    got = col(
        values,
        F.aggregate("v", F.lit(0).cast("long"), lambda acc, x: acc + plus_one(x), lambda a: a * 10),
    )
...
def test_array_sort_by_udf_key(spark):
    ...
    ascending = F.array_sort(
        "v",
        lambda a, b: F.when(plus_one(a) < plus_one(b), -1)
...
def test_transform_values(spark):
    ...
    assert col(df, F.transform_values("m", lambda k, v: plus_one(v))) == [{"a": 2, "b": 3}]


def test_transform_keys(spark):
    df = spark.createDataFrame([({"a": 1, "b": 2},)], "m map<string,int>")
    assert col(df, F.transform_keys("m", lambda k, v: upper(k))) == [{"A": 1, "B": 2}]


def test_map_filter(spark):
    df = spark.createDataFrame([({"a": 1, "b": 2},)], "m map<string,int>")
    assert col(df, F.map_filter("m", lambda k, v: plus_one(v) > 2)) == [{"b": 2}]
...
def test_map_zip_with(spark):
    ...
    got = col(df, F.map_zip_with("m1", "m2", lambda k, v1, v2: add(v1, v2)))
...
def test_udf_on_accumulator_works_and_warns(spark):
    df = spark.createDataFrame([([3, 1, 2],)], "v array<int>")
    expression = F.aggregate("v", F.lit(0).cast("long"), lambda acc, x: plus_one(acc) + x)
...
def test_pairwise_comparator_works_and_warns(spark):
    ...
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        expression = F.array_sort("v", lambda a, b: compare(a, b))
        assert any(w.category is RuntimeWarning for w in caught)
...
def test_fast_paths_do_not_warn(values):
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        F.transform("v", lambda x: plus_one(x))
        F.aggregate("v", F.lit(0).cast("long"), lambda acc, x: acc + plus_one(x))
...
def test_python_if_on_udf_result_is_rejected(values):
    ...
    with pytest.raises((PySparkValueError, ValueError, TypeError)):
        values.select(F.transform("v", lambda x: 1 if plus_one(x) else 0)).collect()
